...
var entryobj:PageObj = {};
var entry:Entry[]= [];
Object.defineProperty(entryobj, "entry", {
    set: function (value) {
        entry = value;
        ipc.send('arrchange', entry);
...
function deleteFromList(target:any) {
    color = '';
    flag = false;
...
function contains(arr:string[],arr2:string[]):boolean{
    let isin1=true;
    let isin2=true;
    for(let i of arr){
        if(i in arr2){
...
function handleclassname(clsname:string) {
    let arr:string[] = [];
...
function getOnlyWay(target:any) {
    if (target.className != '') {
...
function changecolor(target:any) {
    ...
    let tmp = [];
    let parent:any = document.querySelector(obj.finder);
